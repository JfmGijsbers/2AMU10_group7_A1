...
def get_legal_moves(board: SudokuBoard,
                    ...
                    col_set: List[Set[int]]) -> Tuple[Tuple[int, int, int], List[Set[int]]]:
    """
    ...
    for empty_cell in empty_cells:
        ...
        for can_val in [*can_vals]:
            if not TabooMove(i_row, i_col, can_val) in taboo_moves:
                ...
                legal_moves.add((i_row, i_col, can_val))
    return legal_moves, lil_num
...
def update_all_moves2(little_num: List[Set[int]], N: int, legal_moves: Set[Move]) -> Tuple[Set[Move], Set[Move]]:
    """
...
def update_all_moves(little_num: List[Set[int]], N: int, legal_moves: Set[Tuple[int, int, int]])\
        -> Tuple[Set[Tuple[int, int, int]], Set[Tuple[int, int, int]], Set[Tuple[int, int, int]], Set[Tuple[int, int, int]]]:
    """
    Convert little_num into a list of Move objects
    It returns 3 different possible moves list:
        1. list of all moves possible
    ...
    for i in range(N * N):
        ...
        if len(little_num[i]) >= 1:
            for val in little_num[i]:
                all_moves.add((row, col, val))
            if len(little_num[i]) == 1:
                # for val in little_num[i]:
                #     ntab_moves.add(Move(row, col, val))
                ntab_moves.add((row, col, *little_num[i]))
            elif len(little_num[i]) > 1:
                for val in [*little_num[i]]:
                    ptab_moves.add((row, col, val))
    tab_moves = legal_moves.difference(ntab_moves.union(ptab_moves))
    return all_moves, ntab_moves, ptab_moves, tab_moves


def get_all_moves(board: SudokuBoard, played_taboo_moves: List[TabooMove])\
        -> Tuple[List[Tuple[int, int, int]], List[Tuple[int, int, int]], List[Tuple[int, int, int]], List[Tuple[int, int, int]]]:
    """
