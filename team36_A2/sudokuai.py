...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        empty_squares = set(
            [(i, j) for i in range(N) for j in range(N) if game_state.board.get(i, j) == SudokuBoard.empty])
        ...
        for i in range(1, MAX_DEPTH):
            ...
            best_move, eval = self.minimax(game_state, i, float("-inf"), float("inf"), True, 0, empty_squares, moves,
                                           True, taboo)
    ...
    def propose_taboo_move(self, eval, empty_squares):
        ...
        # If you are play on the oneven (winning) side, but there is one taboo move left
            ...
            alternative_moves = [move for eval, move in self.last_moves
                                 if (move.i == taboo_move.i) and (move.j == taboo_move.j) and (
                                             move.value != taboo_move.value)
                                 ]
