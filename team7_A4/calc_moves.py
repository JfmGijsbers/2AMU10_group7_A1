...
# 1: 1 empty cell left
# 2: 2 empty cells left
# 3: 3 empty cells left
# 4: 4 empty cells left
...
def update_all_moves(little_num: List[Set[int]], N: int, legal_moves: Set[Move]) -> Tuple[
    ...
    for i in range(N * N):
        ...
        if len(little_num[i]) >= 1:
            ...
            #     cand_moves.add(Move(row, col, val))
            if len(little_num[i]) == 1:
                # for val in little_num[i]:
    ...
    return ntab_moves, ptab_moves, tab_moves


def get_all_moves(board: SudokuBoard, taboo_moves: List[TabooMove], strategies: bool) -> Tuple[
    Set[Move], Set[Move], Set[Move]]:
    """
    ...
    if strategies:
        little_num = hidden_singles(board.m, board.n, lil_num)
    ...
    ntab_moves, ptab_moves, tab_moves = update_all_moves(lil_num, board.N, legal_moves)
